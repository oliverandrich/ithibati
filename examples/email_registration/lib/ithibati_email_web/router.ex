defmodule IthibatiEmailWeb.Router do
  use IthibatiEmailWeb, :router
  ...
  pipeline :browser do
    ...
    plug :put_root_layout, html: {IthibatiEmailWeb.Layouts, :root}
  ...
  scope "/auth" do
    ...
    # The passkey dialog names this example. The relying-party id is still the host, so the
    # examples share localhost's credential scope even though their databases differ.
    ithibati_routes handler: IthibatiEmailWeb.Auth, rp_name: "Ithibati, email registration"
  end

  scope "/", IthibatiEmailWeb do
  ...
  if Application.compile_env(:ithibati_email, :dev_routes, false) do
    scope "/dev" do
      pipe_through :browser
