...
config :ithibati_email, IthibatiEmail.Repo,
  username: "postgres",
  password: "postgres",
  hostname: "localhost",
  database: "ithibati_email_test#{System.get_env("MIX_TEST_PARTITION")}",
  pool: Ecto.Adapters.SQL.Sandbox,
  pool_size: System.schedulers_online() * 2

# The browser tests drive a real Chrome, so the server is up during `mix test` — which also means
# this port is bound. Each example has a different default port so suites can run concurrently;
# `PORT` still overrides it, as in `config/dev.exs`.
...
config :ithibati_email, :sql_sandbox, true
...
config :ithibati_email, IthibatiEmail.Mailer, adapter: Swoosh.Adapters.Test
