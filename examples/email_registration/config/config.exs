...
config :ithibati,
  repo: IthibatiEmail.Repo,
  user_schema: IthibatiEmail.Accounts.User,
  invitation_schema: IthibatiEmail.Accounts.Invitation,
  users_key_type: :id

config :ithibati_email,
  ecto_repos: [IthibatiEmail.Repo],
  generators: [timestamp_type: :utc_datetime_usec]

config :ithibati, :invitation_mail,
  enabled: false,
  content: &IthibatiEmail.InvitationEmail.content/2,
  deliver: &IthibatiEmail.InvitationEmail.deliver/2

config :swoosh, :api_client, false

# Configures the endpoint
config :ithibati_email, IthibatiEmailWeb.Endpoint,
  url: [host: "localhost"],
  adapter: Bandit.PhoenixAdapter,
  render_errors: [
    formats: [html: IthibatiEmailWeb.ErrorHTML, json: IthibatiEmailWeb.ErrorJSON],
    layout: false
  ],
  pubsub_server: IthibatiEmail.PubSub,
