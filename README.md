...
which is why the first one so rarely gets reused. Ithibati answers only the first:
...
- revocable server-side sessions: the cookie carries a secret, the database carries its digest,
  and signing out revokes the row rather than forgetting it
...
An account is identified by a username or an email address — whichever your application uses. Two
patterns ship, `Identifier.username_format/0` and `Identifier.email_format/0`, and you can pass
your own. You declare the field on your own schema.

Open registration and invitation-only both work, and Ithibati does not push you either way.
Ithibati sends no mail, so delivering an invitation link is your job — and an invitation token is
a bearer secret, so whoever has the link accepts it. That also answers address verification: an
...
Five touchpoints, and they are the whole surface. [Getting
...
There is no password field anywhere, and no sign-in form either: a sign-in challenge names no
credential, so the browser offers whichever passkeys it holds for your site and the person picks
one. Registering is the half that needs a form, because the name does not exist yet.
...
Phoenix, LiveView and Plug are optional dependencies, and the web half comes with them: the
ceremony routes, a gate that answers who is signed in, and a browser hook. Most applications will want that, and
both examples are built that way.

You can also skip it. A ceremony is a handful of function calls — the controller only adds HTTP
and somewhere to park the challenge between two requests. The browser side works without a
framework too: `priv/static/ithibati.js` exports `register` and `authenticate` as plain functions,
and the LiveView hook just wraps them. Use this for a native client, a browser extension, or a
server that is not Phoenix. [Registering and signing
in](https://hexdocs.pm/ithibati/ceremonies.html) shows both ways.
...
Two example applications that compile and run, one for each way of letting people in. Read these
when the docs and your editor disagree:
