...
Two problems, one question — who is allowed to become an account — and Ithibati answers it
...
the invitation — treat that as advice rather than a guarantee, because the guarantee is the
...
identifier is `citext` — which [Getting started](getting_started.md#3-the-account-schema) says
most applications do not need — write the four `add` lines out instead, so the two tables agree
...
way, so a `token_hash` written `:string` is refused at `mix ecto.migrate` rather than at the
first invitation.

### If Ithibati's migration has already run

Which it has, if you followed [Getting started](getting_started.md) — and that changes one
...
people register in the same second rather than one after the other.
...
`register/4` is rewritten rather than added to. The one you have builds its `Ecto.Multi` inline
...
> one sentence, and it is the reason the subject is carried rather than re-read.
...
composing acceptance somewhere that did *not* go through a ceremony — a console, an admin
action — where there is no approved subject to prefer.

`Invitations.account_attrs/1` hands you the identifier the invitation was addressed to, so the
account is created with the address somebody was invited at rather than one typed into the form.
...
`identifier_mismatch` and `already_claimed` — and the `message/1` in your `SignInLive` from
...
is the argument, and it is also where address verification is described as planned rather than
refused.
...
The table is yours with Ithibati's invariants on it, rather than a table Ithibati owns, for the
