...
You need Elixir 1.18 or newer, PostgreSQL, Phoenix 1.8 and LiveView 1.1. The examples below use
the layouts, components and asset configuration generated by Phoenix 1.8. For SQLite, generate
the project with `--database sqlite3`; for MySQL, use `--database mysql`. Apply the corresponding
[database settings](configuration.md#databases) before migrating.
...
Keep `version: 3` pinned. The account table must exist before this migration runs.
